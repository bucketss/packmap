...
```
usage: packmap <mapname.bsp> [mapname2.bsp ...] [folder ...] [--out/-o output_dir] [--res-only/-r]
```
...
use `--res-only`/`-r` to write only `mapname.res`, even when the map is inside a server
